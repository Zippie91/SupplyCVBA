...
$query_ProdBoven2 = sprintf("SELECT * FROM products WHERE products.prod_categorie = %s AND products.prod_plaats= 'boven' AND products.prod_kleur ='2' AND products.prod_actief = '1'", GetSQLValueString($prod_var_ProdBoven2, "int"));
...
mysql_select_db($database_connect_supply, $connect_supply);
$query_ProdOnder2 = sprintf("SELECT * FROM products WHERE products.prod_plaats = 'onder' AND products.prod_categorie = %s AND products.prod_kleur ='2' AND products.prod_actief = '1'", GetSQLValueString($prod_var_ProdOnder2, "int"));
...
<table class="bestelling">
    <thead>   
      <?php $i = kleur_query_2($kleur2, $database_connect_supply, $connect_supply); ?>
    </thead>
    <tbody>
      <tr class="title">
      ...
      <tr class="<?php if($row % 2 == 0) echo 'oneven'; else echo 'even'; ?>">
        ...
        <?php do { ?>
        <td><input id="<?php echo ($row + 1) . '_' . ($col + 1); ?>" name="<?php echo $row_ProdBoven2['prod_id'] . '_' . ($col + 1); ?>" type="text" value="0" size="1" /></td>
        <?php $col++; ?>
        <?php } while ($col < $i); ?>
      ...
      <?php kleur_query_2($kleur2, $database_connect_supply, $connect_supply); ?>
      <tr class="title">
      ...
      <tr class="<?php if($row % 2 == 0) echo 'oneven'; else echo 'even'; ?>">
        ...
        <?php do { ?>
        <td><input id="<?php echo ($row + 1) . '_' . ($col + 1); ?>" name="<?php echo $row_ProdOnder2['prod_id'] . '_' . ($col + 1); ?>" type="text" value="0" size="1" /></td>
        <?php $col++; ?>
        <?php } while($col < $i); ?>
        <td id=<?php echo "rowtotal-" . ($row + 1); ?>>0</td>
      </tr>
